...
struct Ponto
{
  ...
  {
    return id == outro.id; // Compara os IDs dos Pontos
  }

  bool operator!=(const Ponto &outro) const
  {
    return !(*this == outro); // Usa o operador == para verificar desigualdade
  }

  // Sobrecarga para comparar Ponto com IDPonto
  bool operator==(const IDPonto &outro) const
  {
    return id == outro; // Compara o ID do Ponto com o IDPonto
  }
};
...
struct Rota
{
  ...
  {
    return this->id == outra.id; // Compara os IDs das rotas
  }

  bool operator!=(const Rota &outra) const
  {
    return !(*this == outra); // Usa o operador == para definir !=
  }

  // Sobrecarga para comparar Rota com IDRota
  bool operator==(const IDRota &outro) const
  {
    return id == outro; // Compara o ID da Rota com o IDRota
  }
};
