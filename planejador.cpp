...
Ponto Planejador::getPonto(const IDPonto &Id) const
{
  ...
  auto it = std::find_if(pontos.begin(), pontos.end(), [&Id](const Ponto &p)
                         { return p.id == Id; });
...
Rota Planejador::getRota(const IDRota &Id) const
{
  ...
  auto it = std::find_if(rotas.begin(), rotas.end(), [&Id](const Rota &r)
                         { return r.id == Id; });
                     ...
                     const std::string &arq_rotas)
{
  ...
  try
  {
    ...
    do
    {
      ...
      // Caso exista, throw 8
      auto itr_ponto = find_if(listP.begin(), listP.end(), [&P](const Ponto &p)
                               { return p.id == P.id; });
  ...
  try
  {
    ...
    do
    {
      ...
      itr_ponto = find_if(listP.begin(), listP.end(), [&R](const Ponto &p)
      ...
      itr_rota = find_if(listR.begin(), listR.end(), [&R](const Rota &r)
                         { return r.id == R.id; });
                                  ...
                                  Caminho &C, int &NA, int &NF)
{
  ...
  try
  {
    ...
    // Inicializa o conjunto Aberto e Fechado
    list<Noh> Aberto;
    list<Noh> Fechado;
    ...
    while (!Aberto.empty())
    {
      // Ordena os nós em Aberto por custo total e escolhe o primeiro nó (de menor custo)
      ...
      if (atual.id_pt == id_destino)
      {
        ...
        while (!atual.id_rt.empty())
        {
          C.push_front({atual.id_rt, atual.id_pt});
          auto it = find_if(Fechado.begin(), Fechado.end(),
                            [&atual](const Noh &noh)
                            { return noh.id_pt == atual.id_pt; });
          if (it == Fechado.end())
            throw 6;
          atual = *it;
        }
        C.push_front({IDRota(), id_origem});
        NA = Aberto.size();
        NF = Fechado.size();
    ...
    // Se não encontrou o caminho, lança exceção
    throw 7;
